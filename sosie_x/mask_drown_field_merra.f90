PROGRAM mask_drown_field
  ...
  LOGICAL :: l_isleap = .FALSE.
  LOGICAL :: l_K2C = .FALSE.
  LOGICAL :: l_mb2Pa = .FALSE.
  ...
  CHARACTER(LEN=2), DIMENSION(18), PARAMETER :: &
       &  clist_opt = (/ '-h','-v','-k','-x','-y','-z','-t','-w','-i','-m','-q','-p','-g','-o','-M','-D','-l','-u' /)
  ...
  READ(cv_how_far, '(i1)') i_how_far
  IF ( l_drwn_f ) WRITE (*, '("How far (points) from sea to extrapolate into land ",i1)')  i_how_far
  ...
  DO jt = 1, nt
     ...
     IF ( l_drwn_f ) THEN
        ...
        DO jk = 1, nk
           CALL DROWN(iewper, DATA(:,:,jk), mask(:,:,jk), nb_inc=i_how_far, nb_smooth=10)
        END DO
     END IF
     ...
     IF ( l3d ) THEN
        CALL P3D_T(ifo, ivo, nt, jt, xlon, xlat, vdpth, vtime, data, &
             &     cf_out, cv_lon, cv_lat, cv_z, cv_to, cv_out, cu, clnm, rmv)
     ELSE
        CALL P2D_T(ifo, ivo, nt, jt, xlon, xlat,        vtime, data(:,:,1), &
             &     cf_out, cv_lon, cv_lat,          cv_to, cv_out, cu, clnm, rmv)
     END IF
