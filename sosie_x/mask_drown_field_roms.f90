PROGRAM mask_drown_field
  ...
  REAL(4), PARAMETER :: rmv = 1.e+15
  CHARACTER(len=256), PARAMETER :: days_since = 'days since 1900-01-01 00:00:00'
  LOGICAL :: l_isleap = .TRUE.
  ...
  CHARACTER(LEN=2), DIMENSION(17), PARAMETER :: &
       &  clist_opt = (/ '-h','-v','-k','-x','-y','-z','-t','-w','-i','-m','-q','-p','-o','-M','-D','-l','-u' /)
  ...
  DO WHILE ( jarg < iargc() )
     ...
     CASE('-k')
        CALL GET_MY_ARG('output variable', cv_out, 1)
     ...
     CASE('-w')
        CALL GET_MY_ARG('time output', cv_to, 1)
  ...
  READ(cv_ewp, '(i1)') iewper
  IF ( l_drwn_f ) PRINT *, 'East-West periodicity is =', iewper
  ...
  DO jt = 1, nt
     ...
     IF ( l_drwn_f ) THEN
        ...
        DO jk = 1, nk
           CALL DROWN(iewper, data(:,:,jk), mask(:,:,jk))
        END DO
     END IF
...
cv_lat='lat'
     ...
     IF ( l3d ) THEN
        CALL P3D_T(ifo, ivo, nt, jt, xlon, xlat, vdpth, vtime, data, &
             &     cf_out, cv_lon, cv_lat, cv_z, cv_to, cv_out, cu, clnm, rmv, days_since)
     ELSE
        CALL P2D_T(ifo, ivo, nt, jt, xlon, xlat,        vtime, data(:,:,1), &
             &     cf_out, cv_lon, cv_lat,          cv_to, cv_out, cu, clnm, rmv, days_since, lleap=l_isleap)
     END IF
