...
# read data
    tim <- "his"
    for (i in 3:3){
      print(paste("/Users/elizabethdrenkard/TOOLS/CCS_scripts/sardine_GAM/CCS_MAM_",tim,"_EKE_",sprintf("%04d",i),".nc",sep=""))
      sstekefile <- nc_open(paste("/Users/elizabethdrenkard/TOOLS/CCS_scripts/sardine_GAM/CCS_MAM_",tim,"_EKE_",sprintf("%04d",i),".nc",sep=""))
      chlafile <- nc_open(paste("/Users/elizabethdrenkard/TOOLS/CCS_scripts/sardine_GAM/SeaWiFS_CCS_",tim,"_Clim_MAM.nc",sep=""))
      ...
      dteindex <- 1
      dte <- ncvar_get(sstekefile, 'ocean_time')[dteindex]
  
      sst <- ncvar_get(sstekefile, 'SST', start=c(1, 1, dteindex),
                       count=c(-1, -1, 1))
      logchla <- log(ncvar_get(chlafile, 'Chla',
                               start=c(1, 1),
                               count=c(-1, -1)))
      eke <- ncvar_get(sstekefile, 'EKE', start=c(1, 1, dteindex),
                       count=c(-1, -1, 1))
      # SST/EKE w/o _FillValue attribute
      #sst <- replace.fillvalues(sst, sstekefile, 'SST')
      sst[sst == max(sst)] <- NA
      
      logchla <- replace.fillvalues(logchla, chlafile, 'Chla')
      logchla[logchla == -Inf] <- NA
      #eke <- replace.fillvalues(eke, sstekefile, 'EKE')
      eke[eke == max(eke)] <- NA
      
      
      # note I assumed dimensions and pixel locations are the same for
      # all three predictor variables
      #lons <- ncvar_get(chlafile, 'lon')
      #lats <- ncvar_get(chlafile, 'lat')
      lons <- 1:dim(sst)[1] 
      lats <- 1:dim(sst)[2]
  ...
  # predict on Karen's model
      load('/Users/elizabethdrenkard/TOOLS/CCS_scripts/sardine_GAM/R_CODE_SAMPLE/cufesmodel.rda')
  ...
  # If you want to go back to ncdf with predictions:
      ...
      preds <- ncvar_def('prediction', units='probability, 0 to 1', 
                          ...
                          compression=9, chunksizes=c(1, length(londim$vals),
                                                      length(latdim$vals)))
      predfile <- nc_create(paste("/Users/elizabethdrenkard/TOOLS/CCS_scripts/sardine_GAM/sardine_predictions_",tim,"_",sprintf("%04d",i),".nc",sep=""), preds, force=T)
