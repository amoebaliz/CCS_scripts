...
fid1 = nc.Dataset(nc1)
sst1 = np.mean(fid1.variables['SST'][:],axis=0).squeeze()

fid2 = nc.Dataset(nc2)
sst2 = np.mean(fid2.variables['SST'][:],axis=0).squeeze()

sst_dif = sst2-sst1

max_change = np.max(np.abs(sst_dif))
his_range = np.max(sst1) - np.min(sst1)
...
m.drawmeridians([-132,-116], labels=[0,0,0,0], fmt='%d', fontsize=18,zorder=map_order+5)
m.drawparallels([24,38], labels=[0,0,0,0], fmt='%d', fontsize=18,zorder=map_order+5)

im3 = m.pcolormesh(plon,plat,sst_dif,vmin=2,vmax=4,cmap='nipy_spectral',zorder=map_order)
cbar = m.colorbar(im3, location='bottom',size="5%", pad="3%",ticks=[2,2.5,3,3.5,4])

plt.savefig('geo_box.png')
#plt.savefig('Delta_MAM_SST.png')
plt.show()
