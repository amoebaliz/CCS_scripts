...
fid1 = nc.Dataset(nc1)
chl1 = ma.array(fid1.variables['Chla'][:].squeeze())
chl1[chl1==0]=ma.masked

fid2 = nc.Dataset(nc2)
chl2 = ma.array(fid2.variables['Chla'][:].squeeze())
chl2[chl2==0]=ma.masked
...
im3 = m.pcolormesh(lon,lat,chl_dif,vmin=-.01,vmax=.01,cmap='PRGn',zorder=map_order)
cbar = m.colorbar(im3, location='bottom',size="5%", pad="3%",ticks=[2,2.5,3,3.5,4])
m.contour(lon,lat,chl_dif,0,linestyles='dashed',zorder=map_order+6)

plt.savefig('Delta_MAM_Chl.png')
plt.show()
