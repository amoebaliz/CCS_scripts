...
def fill_CA_Gulf(field):
    A = 67
    B = 66*np.ones(3,dtype=np.int)
    BB = 65*np.ones(3,dtype=np.int)
    C = 64*np.ones(2,dtype=np.int)
    nc = np.append(A,np.append(B,np.append(BB,C)))    
    for nr in range(24,32+1):
        
        field[nr,nc[nr-24]:]=ma.masked
...
def get_vel(i):
    ...
    fid = nc.Dataset(ncfile)
    u = np.squeeze(fid.variables['u'][0,0,:])
    u=ma.masked_where(u>100,u)
    print type(u)
    u = fill_CA_Gulf(u)
    v = np.squeeze(fid.variables['v'][0,0,:])
    v=ma.masked_where(v>100,v)
    v = fill_CA_Gulf(v)
    lat = gfid.variables['geolat_c'][:]
    lon = gfid.variables['geolon_c'][:]
    lon[lon>180] = lon[lon>180]-360
    mag = np.sqrt((u)**2+(v)**2)
...
#u,v,mag,lat,lon,mon = get_vel(0)
#im1 = m.pcolor(lon,lat,mag,vmin=-.05,vmax=.05,cmap='nipy_spectral',zorder=map_order)
